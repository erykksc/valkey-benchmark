...
def parse_filename(filename):
    ...
    threads = int(match_threads.group(1)) if match_threads else 1 # Default to 1 if not found
    
    return node_count, cores_str, threads

def get_config_group(node_count, cores_str):
    if node_count == 1 and cores_str == '1cores': return 'Budget Entry Level', 'Single'
    
    if node_count == 1 and cores_str == '4cores': return 'Entry Level', 'Single'
    if node_count == 3 and cores_str == '1cores': return 'Entry Level', 'Cluster'
    
    if node_count == 1 and cores_str == '16cores': return 'Mid Range', 'Single'
    if node_count == 7 and cores_str == '2cores': return 'Mid Range', 'Cluster'
    
    if node_count == 1 and cores_str == '48cores': return 'High Perf', 'Single'
    if node_count == 24 and cores_str == '2cores': return 'High Perf', 'Cluster (24x)'
    if node_count == 48 and cores_str == '1cores': return 'High Perf', 'Cluster (48x)'
...
def process_file(filepath, start_timestamp):
    ...
    try:
        ...
        for chunk in pd.read_csv(filepath, comment='#', chunksize=chunk_size, usecols=['FinishedAt', 'Latency(us)']):
            # Calculate Relative Time
            chunk['RelativeTime'] = (chunk['FinishedAt'] - start_timestamp).astype(int)
            
            # Filter Time Window
            mask = (chunk['RelativeTime'] >= TRIM_START) & (chunk['RelativeTime'] < TRIM_END)
...
def process_data(results_dir=RESULTS_DIR):
    ...
    for (category, sub_label), file_info_list in groups.items():
        ...
        if len(file_list) < 3:
            print(f"Warning: Group {category} - {sub_label} has {len(file_list)} runs (expected 3).")
        ...
        for filepath in file_list:
            try:
                # Get Start Time
                df_head = pd.read_csv(filepath, comment='#', nrows=1, usecols=['FinishedAt'])
                if df_head.empty:
...
def generate_markdown_table(results, output_dir=OUTPUT_DIR):
    ...
    with open(filepath, 'w') as f:
        ...
        f.write("| Category | Configuration | Node Count | Throughput (RPS) | Latency p50 (ms) | Latency p99 (ms) | Latency p99.9 (ms) | Efficiency (RPS/$)\n")
        f.write("|---|---|---|---|---|---|---|---|\n")
        ...
        for category, sub_label in sorted_keys:
            ...
            f.write(f"| {category} | {sub_label} | {data['node_count']}x {data['cores_str']} | {rps_val} | {p50_val} | {p99_val} | {p999_val} | {eff_str} |\n")
...
def generate_plots(results, output_dir=OUTPUT_DIR):
    ...
    for cat in categories:
        ...
        for idx, ((_, label), data) in enumerate(cat_data.items()):
            ...
            ax1.plot(seconds, mean_rps, label=f"{label} RPS", color=color, linestyle='-')
            ax1.fill_between(seconds, 
                             np.array(mean_rps) - np.array(std_rps), 
                             np.array(mean_rps) + np.array(std_rps), 
                             color=color, alpha=0.1)
            
            ax2.plot(seconds, mean_p99, label=f"{label} P99 Latency", color=color, linestyle='--')
        ...
        plt.savefig(os.path.join(output_dir, f'workload_transition_{cat.replace(" ", "_")}.png'))
        plt.close()
        ...
        plt.savefig(os.path.join(output_dir, f'latency_cdf_{cat.replace(" ", "_")}.png'))
        plt.close()
