...
if gcloud compute ssh valkey-sut-node-1 --zone=us-west1-c --command "ls /valkey-installed >/dev/null 2>&1"; then
...
if gcloud compute ssh valkey-client-node --zone=us-west1-c --command "ls /done >/dev/null 2>&1"; then
